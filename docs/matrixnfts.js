	...
	for(var i = 0; i < usersRankings.length; i++){
		...
		$('#rarityResults').append(`
			<tr>
				...
				<td>${ranking.rarestAttributeValue}</td>
				<td><strong>${ranking.rank}</strong> / ${ranks.length} &nbsp <sup>Score: ${parseFloat("" + ranking.avgRatio).toFixed(5)}</sup></td>
				<td><a class="button small" href="${ranking.image}" target="_blank"><i class="fas fa-download"></i></a></td>
				<td><a class="button small" href="https://niftys.com/nft/0x39ceaa47306381b6d79ad46af0f36bc5332386f2/${ranking.tokenId}" target="_blank"><i class="fas fa-arrow-right"></i></a></td>
			</tr>
		`);
	}
}
